...
def spawn_enemy(delay):
    if delay > 5000:
        delay = 0
        Enemy = random.choice([Hole])  # после реализации остальных классов их нужно будет добавить в функцию спавна
...
def main():
    ...
    while True:
        ...
        hit_enemy = pg.sprite.spritecollide(doodle, enemies, False)  # 1
        if hit_enemy:  # 1
            doodle.kill()  # 1
        ...
        if doodle.speed < 0 and doodle.rect.bottom < H / 2:
            ...
            for e in enemies:  # 2
                e.rect.y -= doodle.speed  # 2
        passed_time = spawn_enemy(passed_time) #3
